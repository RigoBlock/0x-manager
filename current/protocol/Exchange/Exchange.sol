...
import { Ownable as Owned } from "../../utils/Ownable/Ownable.sol";
...
contract Exchange is SafeMath {
    ...
    */
    ...
    // GENERALIZED FUNCTIONS FOR ALLOWING FOR MORE INPUT PARAMETERS
...
/*
    ...
    /// @param orderValues Array of order's makerTokenAmount, takerTokenAmount, makerFee, takerFee, expirationTimestampInSec, and salt.
    /// @return Keccak-256 hash of order.
    function getOrderHash(address[] orderAddresses, uint[] orderValues)
        public
        constant
        returns (bytes32)
    {
  ...
  */
  ...
  /*address theSigner;
        if (hasOwner(target)) {
            theSigner = getDragoOwner(target);
        } else {
            theSigner = target;
        }
        return theSigner;*/